...
namespace audio {
  ...
  void RealFft(vector<Real> *v, bool forward) {
    ...
    if (!forward) {
      ComplexFft(v, false);
      // v->Scale(2.0);  // This is so we get a factor of N increase, rather than N/2
                      // which we would
...
at::Tensor fft(at::Tensor input) {
  ...
  torch::Tensor out = torch::rand({input_a.size(0), input_a.size(1)/2 + 1, 2}, torch::TensorOptions().dtype(torch::kFloat32));
